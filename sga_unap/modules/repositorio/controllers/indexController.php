...
class indexController extends repositorioController
{
    private $_repositorioM;
    
    public function __construct($lang,$url) 
    {
        parent::__construct($lang,$url);
        $this->_repositorioM = $this->loadModel('index');        
    }
    ...
    public function roles()
    {
        ...
        $arrayRowCount = $this->_repositorioM->getRolesRowCount($condicion);
        $totalRegistros = $arrayRowCount['CantidadRegistros'];

        // $this->_view->assign('roles', $paginador->paginar($this->_repositorioM->getRoles(), "listarRoles", "$nombre", $pagina, 25));

        $this->_view->assign('modulos', $this->_repositorioM->getModulos(0,0));
        $this->_view->assign('roles', $this->_repositorioM->getRolesPaginado($pagina,CANT_REG_PAG,$soloActivos));
    ...
    public function nuevo_role($usu=false)
    {
        ...
        if($this->_repositorioM->verificarRol($this->getSql('nuevoRol')))
        {
            $this->_view->assign('_error', 'El rol <b style="font-size: 1.15em;">'.$this->getSql('nuevoRol').'</b> ya existe');
        }
        else
        {            
            $idRol = $this->_repositorioM->insertarRol(
    ...
    public function _cambiarEstadoRol()
    {
        ...
        if(!$Rol_IdRol){            
            ...
            $rowCountEstado = $this->_repositorioM->cambiarEstadoRol($Rol_IdRol, $Rol_Estado);
        ...
        $this->_view->assign('roles', $this->_repositorioM->getRolesCondicion($pagina,$filas, $condicion));
    ...
    public function _paginacion_listarRoles($txtBuscar = false) 
    {
        ...
        // $this->_view->assign('roles', $paginador->paginar($this->_repositorioM->getRoles($condicion), "listarRoles", "$nombre", $pagina, 25));
    ...
    public function _buscarRol() 
    {
        ...
        $this->_view->assign('roles', $this->_repositorioM->getRolesCondicion($pagina,CANT_REG_PAG, $condicion));
        $this->_view->assign('numeropagina', $paginador->getNumeroPagina());
        //$this->_view->assign('cantidadporpagina',$registros);
        $this->_view->assign('paginacion', $paginador->getView('paginacion_ajax_s_filas'));
        $this->_view->renderizar('ajax/listarRoles', false, true);
    ...
    public function editarRol($Rol_IdRol = false)
    {
        ...
        $this->_view->getLenguaje("index_inicio");
        $this->_view->setJs(array('index'));
        $rol = $this->_repositorioM->getRol($this->filtrarInt($Rol_IdRol));
        ...
        {            
            ...
            {
                $id = $this->_repositorioM->editarRol($this->filtrarInt($Rol_IdRol), $this->getSql('Rol_Nombre'));
                if($id)
                {
                ...
                $id = $this->_repositorioM->editarTraduccion($this->filtrarInt($Rol_IdRol), $this->getSql('Rol_Nombre'), $this->getSql('idIdiomaSeleccionado'));
                if($id)
                {
        ...
        $this->_view->assign('idiomas',$this->_repositorioM->getIdiomas());        
    ...
    public function gestion_idiomas_rol() 
    {
        ...
        $datos = $this->_repositorioM->getRolTraducido($Rol_IdRol, $Idi_IdIdioma);
    ...
    public function _eliminarRol()
    {
        ...
        if ($Row_Estado == 0) {
            ...
            {            
                ...
                $usu = $this->_repositorioM->getUsuarioRol($Rol_IdRol);
                // print_r($role);
                if ($usu <= 0)
                {                   
                    $rowCount = $this->_repositorioM->eliminarHabilitarRol($Rol_IdRol,$Row_Estado);
                    // echo $rowCount3;//exit;
    ...
    public function permisos($error = "")
    {
        ...
        $arrayRowCount = $this->_repositorioM->getPermisosRowCount($condicion);
        $this->_view->assign('modulos', $this->_repositorioM->getModulos(0,0));
        $this->_view->assign('permisos', $this->_repositorioM->getPermisos($pagina,CANT_REG_PAG,$soloActivos));
    ...
    public function _cambiarEstadoPermisos(){
        ...
        if(!$Per_IdPermiso){            
            ...
            $rowCountEstado = $this->_repositorioM->cambiarEstadoPermisos($Per_IdPermiso, $Per_Estado);
            if ($rowCountEstado > 0) {
                if ($Per_Estado == 1) {
                    $contenido = ' Se cambio de estado correctamente a <b>Deshabilitado</b> <i data-toggle="tooltip" data-placement="bottom" class="glyphicon glyphicon-remove-sign" title="Deshabilitado" style="background: #FFF; color: #DD4B39; padding: 2px;"/> ...!! ';              
                }
        ...
        // echo($totalRegistros);
        // print_r($arrayRowCount); echo($condicion);exit;
        $this->_view->assign('permisos', $this->_repositorioM->getPermisosCondicion($pagina,$filas, $condicion));
    ...
    public function editarPermiso($Per_IdPermiso = false){
        ...
        if ($this->botonPress("bt_editarPermiso")) {
            $id = $this->_repositorioM->editarPermiso($this->filtrarInt($Per_IdPermiso), $this->getSql('permiso_nombre'), $this->getSql('key_'), $this->getInt('modulo_'));
            if($id){
        ...
        $permiso = $this->_repositorioM->getPermiso($this->filtrarInt($Per_IdPermiso)); 
    ...
    public function gestion_idiomas_permisos() {
        ...
        $datos = $this->_repositorioM->getPermisoTraducido($Per_IdPermiso, $Idi_IdIdioma);
        print_r($datos);
        $this->_view->assign('idiomas',$this->_repositorioM->getIdiomas());
        if ($datos["Idi_IdIdioma"]==$Idi_IdIdioma) {
            $this->_view->assign('datos',$datos);    
    ...
    public function _paginacion_listarPermisos($txtBuscar = false) 
    {
        ...
        $paginador = new Paginador();
        // $arrayRowCount = $this->_repositorioM->getPermisosRowCount$arrayRowCount = 0,($condicion);
    ...
    public function _buscarPermiso() 
    {
        ...
        // echo($totalRegistros);
        // print_r($arrayRowCount); echo($condicion);exit;
        $this->_view->assign('permisos', $this->_repositorioM->getPermisosCondicion($pagina,CANT_REG_PAG, $condicion));
    ...
    public function nuevo_permiso()
    {
        ...
        if($this->_repositorioM->verificarPermiso($this->getSql('permiso_')))
        {
            $error = ' Permiso <b style="font-size: 1.15em;"> '. $this->getSql('permiso_').' </b> ya Existe.';
            $i=1;
        }
        
        if($this->_repositorioM->verificarKey($this->getAlphaNum('key_')))
        {
            if($i!=0) 
            {
        ...
        if($i==0)
        {
            $idPermiso = $this->_repositorioM->insertarPermiso(
    ...
    public function _eliminarPermiso()
    {
        ...
        if ($Row_Estado == 0) {
            ...
            {            
                ...
                $role = $this->_repositorioM->verificarPermisoRol($Per_IdPermiso);
                // print_r($role);
                if (!$role)
                {
                    $usuario = $this->_repositorioM->verificarPermisoUsuario($Per_IdPermiso);
                    if(!$usuario){
                        $rowCount = $this->_repositorioM->eliminarHabilitarPermiso($Per_IdPermiso,$Row_Estado);
                        if($rowCount)
                        {
    ...
    public function permisos_role($Rol_IdRol)
    {
        ...
        $row = $this->_repositorioM->getRol($Rol_IdRol);        
        if(!$row)
        {
            $this->redireccionar('acl/roles');
        }
        ...
        if($this->getInt('guardar') == 1)
        {
            ...
            {
                $this->_repositorioM->eliminarPermisoRol(
                        $eliminar[$i]['role'],
                        $eliminar[$i]['permiso']);
            }
            ...
            {
                $this->_repositorioM->editarPermisoRol(
                        $replace[$i]['role'],
                        $replace[$i]['permiso'],
                        $replace[$i]['valor']);
            }
        ...
        $this->_view->assign('permisos', $this->_repositorioM->getPermisosRol($Rol_IdRol));
