...
class indexController extends asistenciasController
{
    private $_aclm;
    ...
    public function asistencias()
    {
        ...
        $condicion   = "ORDER BY caa.Row_Estado DESC ";
        $soloActivos = 0;
        if (!$this->_acl->permiso('ver_eliminados')) {
            $soloActivos = 1;
            $condicion   = " WHERE caa.Row_Estado = $soloActivos ";
        }
        //Filtro por Activos/Eliminados
        $arrayRowCount  = $this->_aclm->getasistenciasRowCount($condicion);
        $totalRegistros = $arrayRowCount['CantidadRegistros'];

        $paginador  = new Paginador();
        $formulario = "asistencias";
        $this->_view->assign('listaDatos', $this->_aclm->getasistenciasPaginado($pagina, CANT_REG_PAG, $soloActivos));
        ...
        $this->_view->assign('numeropagina', $paginador->getNumeroPagina());
        $this->_view->assign('paginacion', $paginador->getView('paginacion_ajax_s_filas'));
        $this->_view->assign('titulo', 'Administración de asistencias');
    ...
    public function registrarAsistencia($id)
    {
        ...
        //Filtro por Activos/Eliminados
        $condicion   = "WHERE mca.Caa_IdCargaAcademica=$id ORDER BY u.Row_Estado DESC ";
        $soloActivos = 0;
        if (!$this->_acl->permiso('ver_eliminados')) {
            $soloActivos = 1;
            $condicion   = " WHERE mca.Caa_IdCargaAcademica=$id and u.Row_Estado = $soloActivos ";
        }
        //Filtro por Activos/Eliminados

        $arrayRowCount  = $this->_aclm->getalumnosRowCount($condicion);
        $totalRegistros = $arrayRowCount['CantidadRegistros'];

        $paginador  = new Paginador();
        $formulario = "alumnos";
        $listaDatos = $this->_aclm->getalumnosPaginado($pagina, 200, $soloActivos, $id);
        $this->_view->assign('listaDatos', $listaDatos);
        ...
        if ($this->botonPress('guardar')) {
            ...
            for ($i = 0; $i < count($listaDatos); $i++) {
                ...
                if (null !== $this->getSql($variable)) {
                    $this->_aclm->registrar_asistencia(1, $this->getSql($variable2), $id);
                } else {
                    $this->_aclm->registrar_asistencia(0, $this->getSql($variable2), $id);
                }
            }

            $this->redireccionar("asistencias/index/asistencias/" . $id);
        }
    ...
    public function registrarAsistenciaDocente()
    {
        $this->_acl->acceso('listar_usuarios');
        $this->validarUrlIdioma();
        $this->_view->getLenguaje("index_inicio");
        ...
        $formulario = "asistenciasdocentes";
        $escuela    = $this->_aclm->getEscuelaXUsuarioRol(Session::get('id_usuario'));
        // // print_r($escuela);
        // $this->_view->assign('semestres', $this->_aclm ->getSemestres());
        // $this->_view->assign('docentes', $this->_aclm ->getDocentes());
        // $this->_view->assign('curriculas', $this->_aclm ->getCurriculasXEscuelaID($escuela['Esc_IdEscuela']) );
    ...
    //Modificado por Jhon Martinez
    public function _buscarasistencias()
    {
        ...
        if ($txtBuscar) {
            ...
            if (!$this->_acl->permiso('ver_eliminados')) {
                $soloActivos = 1;
                $condicion .= " AND caa.Row_Estado = $soloActivos ";
            } else {
                $condicion .= " ORDER BY caa.Row_Estado DESC ";
            }
        ...
        $this->_view->assign('formulario', $formulario);
        $this->_view->assign('listaDatos', $this->_aclm->getasistenciasCondicion($pagina, CANT_REG_PAG, $condicion));
        $this->_view->assign('numeropagina', $paginador->getNumeroPagina());
        $this->_view->assign('paginacion', $paginador->getView('paginacion_ajax_s_filas'));
        $this->_view->renderizar('ajax/' . $nombreLista, false, true);
    }
    //Modificado por Jhon Martinez
    public function _paginacion_listarasistencias($txtBuscar = false)
    {
        //$this->validarUrlIdioma();
        // $pagina = $this->getInt('pagina');
        ...
        // $this->_view->assign('asistencias', $paginador->paginar($this->_aclm->getasistencias($condicion), "listarasistencias", "$nombre", $pagina, 25));

        $paginador->paginar($totalRegistros, $nombreLista, "$txtBuscar", $pagina, $filas, true);
        $usuario = Session::get('id_usuario');
        $this->_view->assign('usuario', $usuario);
        $this->_view->assign('formulario', $formulario);
        $this->_view->assign('listaDatos', $this->_aclm->getasistenciasCondicion($pagina, $filas, $condicion));
        $this->_view->assign('numeropagina', $paginador->getNumeroPagina());
        $this->_view->assign('paginacion', $paginador->getView('paginacion_ajax_s_filas'));
        $this->_view->renderizar('ajax/' . $nombreLista, false, true);
    }
    //Modificado por Jhon Martinez
    public function _cambiarEstadoasistencias()
    {
    ...
    public function _eliminarasistencias()
    {
        ...
        $arrayRowCount  = $this->_indexModel->getasistenciasRowCount($condicion);
        $totalRegistros = $arrayRowCount['CantidadRegistros'];
        ...
        $this->_view->assign('formulario', $formulario);
        $this->_view->assign('listaDatos', $this->_indexModel->getasistenciasCondicion($pagina, $filas, $condicion));
        $this->_view->assign('numeropagina', $paginador->getNumeroPagina());
        $this->_view->assign('paginacion', $paginador->getView('paginacion_ajax_s_filas'));
        $this->_view->renderizar('ajax/' . $nombreLista, false, true);
    }
    ...
    public function verAsistencia($dato)
    {
        $this->_acl->autenticado();
        $this->validarUrlIdioma();
        $this->_view->getLenguaje("index_inicio");
        ...
        if ($this->botonPress('buscar')) {

            $fechas = $this->_aclm->obtener_fechas($this->getSql('fechaI'), $this->getSql('fechaF'), $dato);

            $alumnos = $this->_aclm->obtener_alumnos($this->getSql('fechaI'), $this->getSql('fechaF'), $dato);

            $asistencias = array();

            for ($i = 0; $i < count($alumnos); $i++) {
                $asistencias[$i] = $this->_aclm->obtener_asistencias($this->getSql('fechaI'), $this->getSql('fechaF'), $dato, $alumnos[$i]['Usu_IdUsuario']);
            }

            $this->_view->assign('asistencias', $asistencias);
            $this->_view->assign('fechas', $fechas);
            $this->_view->assign('alumnos', $alumnos);
        }

        $this->_view->renderizar('ver_asistencias');
    }
    ...
    public function registrarSilabo($id)
    {
        ...
        if ($this->botonPress('enviar')) {
            ...
            $this->redireccionar("asistencias/index/registrarSilabo2/" . $id);
        }
        $this->_view->renderizar('agregar_asistencias');
    }
    ...
    public function registrarSilabo2($id)
    {
        ...
        //     else{
        //         $this->redireccionar('asistencias/index/registrarSilabo3/'.$id);
        //     }
        // }
        // $this->_view->renderizar('agregar_asistencias2');
        ...
        if ($this->botonPress('guardar')) {
            ...
            if (count($compCont) != 0) {
                ...
                $this->redireccionar('asistencias/index/editarSilabo2/' . $id);
            }
        }
        ...
        $this->_view->renderizar('editar_asistencias2');
    }
    ...
    public function registrarSilabo3($id)
    {
        ...
        if ($this->botonPress('enviar')) {
            ...
            $this->redireccionar('asistencias/index/registrarSilabo4/' . $id);
        }
        $this->_view->renderizar('agregar_asistencias3');
    }
    ...
    public function registrarSilabo4($id)
    {
        ...
        if ($this->botonPress('enviar')) {
            ...
            $this->redireccionar('asistencias/index/registrarSilabo5/' . $id);
        }
        $this->_view->renderizar('agregar_asistencias4');
    }
    ...
    public function registrarSilabo5($id)
    {
        ...
        if ($this->botonPress('enviar')) {
            ...
            if ($tipo_doc !== "image/jpg" && $tipo_doc !== "image/jpeg" && $tipo_doc !== "image/png") {
                ...
                echo ("parent.location.href='agregar_asistencias4.php';");
                echo ("</script>");
            } else {
                $carpeta = "modules/asistencias/views/index/images/";
                opendir($carpeta);
            ...
            $this->redireccionar('asistencias/index/asistencias');
        }
        $this->_view->renderizar('agregar_asistencias5');
    }
    ...
    public function editarSilabo($id)
    {
        ...
        $this->_view->renderizar('editar_asistencias');
    }
    ...
    public function editarSilabo2($id)
    {
        ...
        if ($this->botonPress('enviar')) {
            ...
            if (count($cap1) == 0 || count($cap2) == 0 || count($cap3) == 0) {
                ...
                $this->redireccionar('asistencias/index/editarSilabo3/' . $id);
            }
        }
        $this->_view->renderizar('editar_asistencias2');
    }
    ...
    public function editarSilabo3($id)
    {
        ...
        if ($this->botonPress('enviar')) {
            ...
            if ($cont == '') {
                $this->redireccionar('asistencias/index/editarSilabo4/' . $id);
            }
        ...
        if ($cont == '') {
            ...
            $this->_view->renderizar('editar_asistencias3');
        }
    ...
    public function editarSilabo4($id)
    {
        ...
        if ($this->botonPress('enviar')) {
            ...
            $this->redireccionar('asistencias/index/editarSilabo5/' . $id);
        }
        $this->_view->renderizar('editar_asistencias4');
    }
    ...
    public function editarSilabo5($id)
    {
        ...
        $this->_view->renderizar('editar_asistencias5');
    }
